  ...
  <div class="announcement">
    ...
    <ul class="an-box clear">
      <li class="alignLeft clear">
        <p class="an-con">
          本站基于 Vue 2.x, 不支持 IE8 及以下版本。 同时使用了大量的 CSS3
          的样式及 HTML5 的 api，如 flex 布局、变换和过渡、history
          路由模式和本地存储等， 经测试 IE11 以下运行都存在问题
        </p>
        <span class="top">
...
export default {
  ...
  methods: {
    ...
    ansSort(arr) {
      ...
      let normal = [];
      arr.forEach(an =>
        an.type === "置顶" ? top.unshift(an) : normal.unshift(an)
      );
    ...
    getAnnouncements() {
      this.$axios
        .get(`${this.$root.cors}/latestAns?count=${this.count}`)
        .then(resp => {
