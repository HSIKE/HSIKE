...
export default {
  ...
  computed: {
    isShow() {
      return this.$root.store.state.isShow;
    },
    alertMsg() {
      return this.$root.store.state.alertMsg;
    },
    turnOff() {
      return this.$root.store.state.count >= 3;
    }
  },
  methods: {
    iKnow() {
    ...
    off() {
      this.$root.store.off.call(this.$root.store);
    }
  }
};
</script>
