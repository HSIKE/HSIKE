<template>
  <div id="notes">
            ...
            <li class="con-item contact">
              ...
              <div class="content contacts">
                <span class="qq" title="QQ" @click="showQQ">
                  <i class="fa fa-qq"></i><br /> </span
                ><span class="wechat" title="微信" @click="showWechat">
                  <i class="fa fa-weixin"></i> </span
                ><span class="email" title="邮箱">
                  <i class="fa fa-envelope-o" aria-hidden="true"></i> </span
                ><a
                  href="https://github.com/HSIKE?tab=repositories"
                  class="github"
                  target="_blank"
                  title="github"
                >
    ...
    <div class="footer">
      <p class="f-head">
        Designed & Coded By
        <a href="mailto:893008093@qq.com" style="font-style: italic"
          >@Mr.Huang</a
        >, 2019 / 02<br />
        Based On
    ...
    <transition name="fast-fade">
      <div class="qr_code" v-show="showContact" @click="hideContact">
        <img
          ref="qr_qq"
          src="./assets/images/qr_code_qq.jpg"
          alt="图片加载失败，刷新试试"
        />
        <img
          ref="qr_wechat"
          src="./assets/images/qr_code_wechat.jpg"
          alt="图片加载失败，刷新试试"
        />
      </div>
    </transition>
...
export default {
  ...
  methods: {
    getNavList() {
      this.$axios
        .get(`${this.$root.cors}/navs/navList`)
        .then(resp => {
          let data = resp.data;
          if (Array.isArray(data)) {
            ...
            this.showAlert("天啦，出bug啦，赶紧点那边的联系方式让人来修吧~");
        })
        .catch(err => this.showAlert("可能是服务器定期重启，刷新试试？"));
    },
    showAlert(msg) {
      this.$root.store.show.call(this.$root.store, msg);
    },
    hideContact() {
